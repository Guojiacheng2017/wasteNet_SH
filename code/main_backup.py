...
data_test = torchvision.datasets.ImageFolder(root=data_path_test, transform=transform)
...
def dataset_sampler_test(dataset):  # test_percentage,
    sample_num = len(dataset)
    file_idx = list(range(sample_num))
    test_idx, val_idx = train_test_split(file_idx, test_size=0.001, random_state=42)
    test_sampler = SubsetRandomSampler(test_idx)
    return test_sampler
...
test_sampler = dataset_sampler_test(data_test)
...
test_loader = torch.utils.data.DataLoader(data_test, sampler=test_sampler)
